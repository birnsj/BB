class_name NpcNpcMoveToPointState
extends NpcState
...
func enter(payload: Variant = null) -> void:
	if payload is Vector2:
		...
		(player as Npc).request_camera_walk_start_recenter()
		return
	push_error(
		"NpcMoveToPointState.enter: expected Vector2 destination, got %s. Falling back to idle." % payload
	)
