class_name PlayerTuningApplier
extends RefCounted
...
static func apply(player: Player, profile: CharacterTuningProfile) -> void:
	...
	if sm:
		...
		var md := sm.get_node_or_null("MouseDrag") as MouseDragState
		if md:
	...
	var atk := player.get_node_or_null("Interactions/AttackBox") as PlayerAttackHitbox
	if atk:
	...
	if cam:
		...
		if rig is CameraLagRig:
			(rig as CameraLagRig).follow_smoothness = profile.follow_smoothness
